...
This workflow ensures that the [Sphinx](https://www.sphinx-doc.org/) documentation builds successfully.  It also publishes the resulting build to [GitHub Pages](https://pages.github.com/) if it is from the appropriate branch (e.g., `stable/0.1`).
...
This workflow is triggered by a maintainer pushing a tag that represents a release.  Because `qiskit-paulice` contains a Rust extension built with [maturin](https://www.maturin.rs/) and [PyO3](https://pyo3.rs/) (using `abi3-py310`), this workflow uses [cibuildwheel](https://cibuildwheel.pypa.io/) to build platform-specific wheels for Linux (`x86_64`, `aarch64`), macOS (`x86_64`, `arm64`), and Windows (`amd64`) in addition to the source distribution.  It then publishes the release to github.com and to [PyPI](https://pypi.org/).
